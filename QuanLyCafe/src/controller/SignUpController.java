...
public class SignUpController implements ActionListener {
    ...
    private void handleSignUp() {
        try {
            ...
            int phoneNumber = Integer.parseInt(signUpView.getTextFieldPhoneNumber().getText());
            String role = signUpView.getSelectedRole();

            if (name.isEmpty() || email.isEmpty() || password.isEmpty() || address.isEmpty() || role.isEmpty()) {
                JOptionPane.showMessageDialog(signUpView, "Bạn Phải Điển Đủ Tất Cả các Ô", "Error", JOptionPane.ERROR_MESSAGE);
                return;
            }
            ...
            ex.printStackTrace();
            JOptionPane.showMessageDialog(signUpView, "Đã Xảy Ra Lỗi Trong quá Trình Đăng Kí: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
}
